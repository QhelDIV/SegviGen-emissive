#!/usr/bin/env python3
"""o_voxel's own GLB exporter, with an emissive channel added.

PROVENANCE. This is o_voxel.postprocess.to_glb copied verbatim from the installed
package and changed in exactly two places, both marked "ADDED (lightgen)":

  source : /cs/3dlg-jupiter-project/lightgen/miniforge3/envs/trellis2/
           lib/python3.10/site-packages/o_voxel/postprocess.py
  md5    : 2fc25fcf1eb056c222a65ea6a06037c2  (checked at import; see _check_upstream below)

WHY A COPY. Upstream's material construction reads four fixed attribute names
(base color, metallic, roughness, alpha) and drops every other channel of the
attribute volume, so an emissive channel cannot be threaded through by argument.
Adding it upstream is a small change to that one block; this copy is the prototype
of that change, kept in the fork so the voxel-native render pipeline can be
evaluated before o_voxel is touched.

WHY NOT TWO PASSES. The obvious workaround, calling upstream to_glb twice with the
emission substituted into the base color slot on the second call, was implemented
first and REJECTED by measurement: the remesh and UV unwrap are not deterministic.
Two calls on identical input vertices and faces produced 56338 and 52205 UVs on the
laptop asset, and different counts again on a rerun. The emission texture would not
have registered with the geometry it was painted on.

DO NOT EDIT the body below except to re-sync it with upstream. If o_voxel changes,
regenerate this file rather than hand-merging; the generator is in this job's
history and the change is small enough to reapply by hand from the two markers.
"""
import hashlib
import os
import warnings
...
UPSTREAM = ("/cs/3dlg-jupiter-project/lightgen/miniforge3/envs/trellis2/"
            "lib/python3.10/site-packages/o_voxel/postprocess.py")
UPSTREAM_MD5 = "2fc25fcf1eb056c222a65ea6a06037c2"


def _check_upstream():
    """A copy that has silently drifted from its source is worse than no copy, so
    say so at import time rather than letting a stale exporter produce results
    nobody can trace back."""
    path = UPSTREAM if os.path.isfile(UPSTREAM) else UPSTREAM.replace("/cs/", "/", 1)
    if not os.path.isfile(path):
        return
    got = hashlib.md5(open(path, "rb").read()).hexdigest()
    if got != UPSTREAM_MD5:
        warnings.warn(
            f"o_voxel.postprocess has changed since this copy was taken "
            f"(md5 {got} vs {UPSTREAM_MD5}); re-sync ovoxel_to_glb_emissive.py")
...
def to_glb_emissive(
    ...
    CHANGED vs upstream: if `attr_layout` carries an 'emissive' slice, those three
    channels are sampled and inpainted exactly like base color and become the
    material's emissive texture, and the texture is also returned so a renderer can
    consume it directly. Everything else is upstream, verbatim.
    Returns (trimesh.Trimesh, PIL.Image or None).
    ...
    # ---- ADDED (lightgen): the emissive channel -----------------------------
    # Same treatment base color gets: clip to 8 bit, then dilate into the UV
    # gutter so a texel that lands just outside a chart does not read as black at
    # the seam. Without the inpaint, emissive edges show a dark fringe exactly
    ...
    material = trimesh.visual.material.PBRMaterial(
        ...
        emissiveFactor=(np.array([1.0, 1.0, 1.0]) if emissive_img is not None else None),
    )
    ...
    return textured_mesh, emissive_img
