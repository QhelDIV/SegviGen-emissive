...
**Script.** `emissive/render/render_emissive.py`, `--mode box`.
...
**Script.** `emissive/render/render_emissive.py`, `--mode method`
(the default mode).
...
**Script.** `emissive/render/strength_ladder.py`, which shells out to
...
1. **Defaults have moved.** `render_emissive.py`'s defaults changed three times
   ...
   render.
2. **Use the supplied cameras.** `emissive/render/cameras/<sid>.json`
...
4. **Compute nodes cannot see the working tree.** The repository is on
   ...
   `emissive/render/example/stage.sh`, and copy GLBs rather than
