#!/usr/bin/env python3
"""Publish web/_preview/paper_v3/ to the aspis web root by MERGE-COPY.
...
Live URL: https://aspis.cmpt.sfu.ca/projects/omages/yanxg/lightgen/_preview/paper_v3/

Run: /cs/3dlg-project/3dlg-hcvc/omages/omages_internal/.venv2/bin/python \
        web/_preview/paper_v3/publish.py
"""
import os
import shutil
...
DEST = os.path.join(WWW, "_preview", "paper_v3")

SKIP = {"__pycache__", "build.py", "publish.py", "make_gallery.py", "gallery.json"}
...
def main():
    ...
    for root, dirs, files in os.walk(HERE):
        ...
        for f in files:
            ...
            n += 1
    # Withdraw anything this page no longer carries. Merge-copy alone cannot
    # unpublish, and a model dropped for its licence has to actually leave the
    # public directory. Scoped to this page's own img/ and glb/, file by file
    # against the local listing: never a directory removal.
    for sub in ("img", "glb"):
