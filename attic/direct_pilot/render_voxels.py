...
def render(coords, mask, out_png, cam=(1.9, -2.3, 1.5), upright=False):
    from xgutils import bpyutil
    from xgutils.vis import visutil
    if upright:
        # Dongchen emission_voxels_256 frame: visual up is the Y (2nd) axis. Rotate
        # +90 about X, (x,y,z)->(x,-z,y). Only for that frame — o_voxel-voxelized
        # panels use a different frame, so this is opt-in. (See software_voxrender.)
        coords = np.stack([coords[:, 0], -coords[:, 2], coords[:, 1]], axis=1)
    V, F, VC, n_emis, n_tot = build_cube_mesh(coords, mask)
    # normalize by the FULL coord bbox (identical for broken/fixed of the same shape,
    # so their panels align perfectly), not the subsampled cube verts.
    call = coords / 512.0 - 0.5
    clo, chi = call.min(0), call.max(0)
    V = (V - (clo + chi) / 2) / (chi - clo).max()
